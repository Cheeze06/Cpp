...
// 1. 사용자로부터 문자열의 개수를 입력받습니다.
// 2. 입력받은 개수만큼 문자열을 동적으로 할당.
// 3. 각 문자열의 최대 길이는 100자로 가정.
// 4. 사용자로부터 각 문자열을 입력받아 할당된 배열에 저장.
// 5. 입력한 문자열들을 출력
// 6. 문자열 사용이 끝난 후에 동적으로 할당한 메모리를 해제.
// 프로그램을 작성하고, 주어진 요구사항을 모두 충족하는지 확인.
// MY
// 문제점: 문자열 최대 100자가 아님
/*
int main()
...
//		stringArray[100];
//		cin >> input;
//		stringArray[i] = new char[strlen(input) + 1];
